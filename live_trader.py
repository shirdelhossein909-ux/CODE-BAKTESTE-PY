...
def sync_all(symbols, reason_txt="بازبینی"):
    """بازپخش استراتژی روی همه‌ی نمادها + سهمیه‌بندی اوردرها:
    هر نماد حداکثر «یک» اوردر (بهترین زونش)، به ترتیب اولویت سبد (طلا اول)،
    تا سقف کل (MAX_PENDING_TOTAL)."""
    ...
    # ۲) سهمیه‌بندی: هر نماد فقط «یک» اوردر (بهترین زونش)، به ترتیب اولویت سبد، تا سقف کل
    alloc = {}
    total = 0
    for b in symbols:
        lst = all_wanted.get(b, [])
        if lst and total < MAX_PENDING_TOTAL:
            alloc.setdefault(b, {})[str(lst[0]["zone_id"])] = lst[0]
            total += 1
