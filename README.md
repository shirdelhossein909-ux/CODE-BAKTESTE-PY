...
6. **شبیه‌سازی «عین لایو» (مهم‌ترین اصلاح):** همه‌ی نمادها **هم‌زمان** روی یک حساب مشترک جلو می‌روند و سقف‌ها دقیقاً همان‌جایی اعمال می‌شوند که ربات اعمال می‌کند — یعنی موقع چیده شدن سفارش، نه بعد از ساخته شدن فهرست معاملات:
   - سهمیه‌بندی دوری (round-robin) بین نمادها، سقف ۸ سفارش در کل حساب، هر نماد حداکثر ۳
   - ربات روی زون‌های **لمس‌نشده** هم از قبل لیمیت می‌چیند؛ این سفارش‌ها جای واقعی از سقف ۸ اشغال می‌کنند و در بک‌تست هم مدل می‌شوند
   - ریسک هر معامله درصدی از اکویتی همان لحظه‌ی حساب مشترک، با وزن سشن
   - قبلاً هر نماد جدا بک‌تست می‌شد و عملاً ۱۲×۳ = ۳۶ سفارش هم‌زمان مجاز بود؛ به همین دلیل بک‌تست بسیار بیشتر از ربات معامله می‌زد.
...
| `LIVE_MODE` | `True` | شبیه‌سازی «عین لایو» (همه‌ی نمادها هم‌زمان روی یک حساب) |
| `LIVE_MAX_PENDING_TOTAL` | `8` | سقف سفارش در انتظار، کل حساب — برابر ربات |
| `LIVE_MAX_OPEN_TOTAL` | `8` | سقف پوزیشن باز، کل حساب — برابر ربات |
| `LIVE_MAX_PENDING_PER_SYMBOL` | `3` | سقف سفارش هر نماد |
| `LIVE_ARM_UNTOUCHED` | `True` | چیدن سفارش روی زون‌های لمس‌نشده (عین ربات) |
| `LIVE_RISK_PER_TRADE` | `0.005` | ریسک هر معامله از اکویتی حساب (نیم درصد) |
| `PORTFOLIO_*` | — | فقط وقتی `LIVE_MODE = False` باشد استفاده می‌شود |
